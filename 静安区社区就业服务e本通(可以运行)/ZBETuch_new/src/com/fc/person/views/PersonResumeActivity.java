...
public class PersonResumeActivity extends Activity implements IActivity {
	private TextView tv_language1, tv_language2, tv_computerskills,
			tv_computerzhengshhu, tv_chendu1, tv_chendu2, tv_languagezhengshu,
			tv_otherzhengshu, tv_selfpingjia, tv_gzxz, tv_xinzi, tv_gzbs,
			tv_address1, tv_address2, tv_address3, tv_fugangwei1,
			tv_zigangwei1, tv_fugangwei2, tv_zigangwei2, tv_othergangwei,
			tv_workyear;
		...
		public void handleMessage(Message msg) {
			...
			//	Log.i("resume.json/....", personResumeJson.toString());
				if (personResumeJson.size() == 0) {
					// Toast.makeText(PersonResumeActivity.this, "没有个人简历信息。。。",
	...
	Runnable thread_Resume = new Runnable() {
		...
		public void run() {
			personResumeJson = HttpUrls_.getPersonResumeJson(
					PersonResumeActivity.this, personSFZ);
	...
	protected void initinfo() {
		...
		for (int i = 0; i < personResumeJson.size(); i++) {
			...
			tv_computerzhengshhu.setText(personResumeJson.get(i)
			...
			tv_languagezhengshu.setText(personResumeJson.get(i)
					.getPersonForeignZhengshu());
			tv_othergangwei.setText(personResumeJson.get(i).getPersonGanwei());
			tv_selfpingjia.setText(personResumeJson.get(i)
					.getPersonAssessment());
			String personExpectedSalarylow = personResumeJson.get(i)
					.getPersonExpectedSalarylow().trim().replaceAll(" ", "");
			String sublow = personExpectedSalarylow.substring(0,
					personExpectedSalarylow.indexOf("."));
			String personExpectedSalaryup = personResumeJson.get(i)
					.getPersonExpectedSalaryUp().trim().replaceAll(" ", "");
			String subup = personExpectedSalaryup.substring(0,
			...
			tv_otherzhengshu.setText(personResumeJson.get(i)
					.getPersonQitazhengshu());
			String personZigangwei1 = personResumeJson.get(i)
					.getPersonZigangwei1();
			...
			String personZigangwei2 = personResumeJson.get(i)
					.getPersonZigangwei2();
			...
			String personworkyears = personResumeJson.get(i)
					.getPersonworkyears();
	...
	private class MyOnClickListener implements OnClickListener {
		...
		public void onClick(View v) {
			switch (v.getId()) {
			case R.id.btnSearch:
				Intent intent = new Intent(PersonResumeActivity.this,
