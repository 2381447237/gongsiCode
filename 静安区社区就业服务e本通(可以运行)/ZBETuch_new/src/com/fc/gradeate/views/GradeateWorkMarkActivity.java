...
public class GradeateWorkMarkActivity extends Activity implements IActivity,
		OnPullDownListener {
				...
				if (infos.size() > 0) {
					for (int i = 0; i < infos.size(); i++) {
						...
						if (jobMarkInfo.getId() == info.getId()) {
							jobMarkInfo.setBase_situation(info
									.getBase_situation());
							jobMarkInfo.setCreat_date(info.getCreat_date());
							jobMarkInfo.setCreat_staff(info.getCreat_staff());
							jobMarkInfo.setDetail_company(info
									.getDetail_company());
							jobMarkInfo.setDetail_situation1(info
									.getDetail_situation1());
							jobMarkInfo.setDetail_situation2(info
									.getDetail_situation2());
	...
	private void initView() {
		...
		mPullDownView = (PullDownView) findViewById(R.id.lvWorkMark);
		...
		public void onClick(View v) {
			...
			case R.id.btnNew:
				...
				if (infos != null && infos.size() > 0) {
					intent.putExtra("lastJobMarkInfo", infos.get(0));
				}
