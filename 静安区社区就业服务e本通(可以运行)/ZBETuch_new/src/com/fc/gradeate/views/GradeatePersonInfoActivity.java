...
public class GradeatePersonInfoActivity extends Activity implements IActivity {
		...
		case GradeatePersonInfoActivity.SAVE_GRADEATEPERSON:
			...
			if (params[1] != null && params[1].toString().trim().length() > 0) {
				...
				CompanyTask task = new CompanyTask(
						CompanyTask.GRADEATE_EDITWORKMARKACTIVITY, param);
	...
	private void saveGradeatePerson() {
		...
		info.setPoliticalStatus(cboPolityStyle.getSelectedItem().toString()
				.trim());
		...
		info.setStreetId(Integer.parseInt(((StreetInformation) cboStreet
	...
	private class MySpinnerItemSelectedListener implements
		...
		public void onItemSelected(AdapterView<?> arg0, View arg1, int arg2,
			...
			switch (arg0.getId()) {
			case R.id.cboStreet:
				street = (StreetInformation) cboStreet.getSelectedItem();
				if (!street.getStreetId().trim().equals("-1")) {
					if (maps.get(street.getStreetId()) != null
							&& !"".equals(maps.get(street.getStreetId()))) {
