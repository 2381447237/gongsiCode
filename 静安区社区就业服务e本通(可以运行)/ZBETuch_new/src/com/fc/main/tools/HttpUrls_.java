...
public class HttpUrls_ {
	//public static String HttpURL = "http://192.168.3.2:89"; //服务器
	public static String HttpURL = "http://web.youli.pw:89"; //服务器
	...
	//  public static  String HttpURL = "http://192.168.4.11:89"; // 3G
	...
	// public static final String HttpURL = "http://10.65.34.240:89";//
	// public static final String HttpURL="http://192.168.191.1:92";
	// public static final String HttpURL = "http://192.168.11.120";
	//public static final String HttpURL = "http://192.168.191.1";
	//public static final String HttpURL = "http://192.168.1.200";
	...
	public static String login(final LoginActivity context, String userName,
			String userPwd, int versionCode) {
		...
		try {
			...
			String code = Fc_Helper.EncryptDES(versionCode + "", msgkey);
		...
		//	Log.i("2017/3/3url+++++++++++++", urllogin);
			Log.i("2018-4-21","====="+urllogin);
			URL url = new URL(urllogin);
	...
	public static ArrayList<CommitteeInformation> getCommitteeJson(
			Context context, String street) throws JSONException {
		...
		try {
			...
			if (conn.getResponseCode() == 200) {
				...
				// 将字节转换成字符串
				msgJson = sb.toString(); // new String(buffer, "utf-8");
				//Log.i("得到的信息。。。。", msgJson);
				JSONArray jsonarray = new JSONArray(msgJson);
				int len = jsonarray.length();
				for (int i = 0; i < len; i++) {
	...
	public static ArrayList<PersonMark> getPersonMarkJson(
			final Context addpersoninfo, String IdCard) throws JSONException,
	...
	public static String postJson1(ArrayList<LocationInformation> list) {
		...
		return "";
		/*
		 * StringBuilder builder = new StringBuilder(); String urlPost = HttpURL
		 * + "/Json/Get_Gps_Info.aspx"; List<NameValuePair> nameValuePair = new
		 * ArrayList<NameValuePair>(); JSONArray jsonarray = new JSONArray();
		 * for (LocationInformation locationInfo : list) { JSONObject jsonObject
		 * = new JSONObject(); try { jsonObject.put("ID", locationInfo.getId());
		 ...
		 * jsonObject.put("CREATE_DATE", locationInfo.getCreate_date()); } catch
		 * (JSONException e) { e.printStackTrace(); } jsonarray.put(jsonObject);
		 * } nameValuePair.add(new BasicNameValuePair("jsonString", jsonarray
		 * .toString())); try { // 建立服务器的连接，以post方式发送到服务器。 HttpClient httpClient
		 * = new DefaultHttpClient(); HttpPost httpPost = new HttpPost(urlPost);
		 * httpPost.setEntity(new UrlEncodedFormEntity(nameValuePair, "utf-8"));
		 * Log.i("json", nameValuePair.toString()); HttpResponse httpResponse =
		 * httpClient.execute(httpPost); Log.i("？？？？？？？",
		 * httpResponse.toString()); BufferedReader reader = new
		 * BufferedReader(new InputStreamReader(
		 * httpResponse.getEntity().getContent())); for (String s =
		 * reader.readLine(); s != null; s = reader.readLine()) {
		 * builder.append(s); } } catch (UnsupportedEncodingException e) { //
		 * TODO Auto-generated catch block e.printStackTrace(); } catch
		 * (ClientProtocolException e) { // TODO Auto-generated catch block
		 * e.printStackTrace(); } catch (IOException e) { // TODO Auto-generated
		 * catch block e.printStackTrace(); } return builder.toString();
		 */
	}
		...
		try {
			...
			if (conn.getResponseCode() == 200) {
				...
				for (int i = 0; i < len; i++) {
					...
					// 转化时间格式
					String substring = lostJobNewTime.substring(0,
							lostJobNewTime.indexOf("T"));
	...
	public static String postImgJson(ArrayList<PersonUpdataImg> list_updataImg,
			ArrayList<LocationInformation> list_location) {
		StringBuilder builder = new StringBuilder();
		...
		/*
		 * for(LocationInformation location: list_location){ JSONObject
		 * jsonObject = new JSONObject(); try { jsonObject.put("jingdu",
		 * location.getLongitude()); jsonObject.put("weidu",
		 * location.getLatitude()); } catch (JSONException e) { // TODO
	...
	public static ArrayList<PendingWorkInformation> getPendingWorkJson(
			Context context, int page, int rows, String typecd)
			throws JSONException, UnsupportedEncodingException {

		String typecode = URLEncoder.encode(typecd, "utf-8");
		// page=0，rows=2主mian日志显示，page=0，rows=15：三星显示，page=0，rows=
		String urlStr = HttpURL + "/Json/Get_PendingWork.aspx?page=" + page
				+ "&rows=" + rows + "" + "&type=" + typecode;
	...
	public static Bitmap getPersonImage(Context context, String Idcard) {
		String urlstr = HttpURL + "/Web/Personal/windows/ShowPic.aspx?sfz="
				+ Idcard;
		Bitmap bitmap = null;
		HttpURLConnection conn = GetConnection(urlstr);
		conn.setConnectTimeout(5000);
	...
	public static String postPersonJson(
			ArrayList<PersonalBaseInformation> list_update) {
		StringBuilder builder = new StringBuilder();
		...
		for (PersonalBaseInformation personupdate : list_update) {
			...
			try {
				...
				jsonObject.put("CULTURAL_CODE",
						personupdate.getPersonEducation());
				jsonObject.put("SFZ", personupdate.getPersonCardId());
				jsonObject.put("CONTACT_NUMBER",
						personupdate.getPersonMobilePhone());
				...
				jsonObject.put("Current_situation",
						personupdate.getPersonCurrentStatus());
				jsonObject.put("Current_intent",
						personupdate.getPersonIntention());
	...
	public static String PendWorkPostJson(ArrayList<PendingWorkInformation> list) {
		...
		new Thread() {
			...
			public void run() {
				...
				try {
					builder.append(HttpUtil.postRequest(
							"/Json/Set_PendingWork.aspx", map));
	...
	public static ArrayList<GetNews> getNewsJson(Context context, int page,
			int rows) throws JSONException, UnsupportedEncodingException {

		// page=0，rows=2主mian日志显示，page=0，rows=15：三星显示，page=0，rows=
		String urlStr = HttpURL + "/Json/Get_News.aspx?page=" + page + "&rows="
				+ rows;
	...
	public static ArrayList<GetMsgBoardMaster> getmsgboardmasterJson(
			Context context, int page, int rows) throws JSONException,
			UnsupportedEncodingException {

		// page=0，rows=2主mian日志显示，page=0，rows=15：三星显示，page=0，rows=
		String urlStr = HttpURL + "/Json/Get_MsgBoard_Master.aspx?page=" + page
				+ "&rows=" + rows;
	...
	public static ArrayList<GetMsgBoardDetile> getmsgboarddetileJson(
			Context context, int masterid) throws JSONException,
			UnsupportedEncodingException {

		// page=0，rows=2主mian日志显示，page=0，rows=15：三星显示，page=0，rows=
		String urlStr = HttpURL + "/Json/Get_MsgBoard_Detile.aspx?"
				+ "master_id=" + masterid;
	 ...
	 * @throws UnsupportedEncodingException
	 */
	public static ArrayList<GetJobs> getjobsJson(Context context, int page,
			int rows) throws JSONException, UnsupportedEncodingException {

		// page=0，rows=2主mian日志显示，page=0，rows=15：三星显示，page=0，rows=
		String urlStr = HttpURL + "/Json/GetJobs.aspx?" + "page=" + page
				+ "&rows=" + rows;
	...
	public static String CheckPersoninfo(String titleName) {
		String urlStr = "";
		try {
			urlStr = HttpURL + "/Json/GetCheckLineLevel.aspx?module_name="
	...
	public static String postResumeJson(ArrayList<PersonResume> list_resume) {
		...
		for (PersonResume personresume : list_resume) {
			...
			try {
				...
				// jsonObject.put("WORKYEARS",
				// personresume.getPersonworkyears());
		...
		Log.e("2017/9/12","修改个人信息"+jsonarray.toString());
	...
	public static ArrayList<PersonResume> getPersonResumeJson(
		...
		try {
			...
			if (conn.getResponseCode() == 200) {
				...
				for (int i = 0; i < len; i++) {
					JSONObject object = jsonarray.getJSONObject(i);
					String personcomputerskills = object
							.getString("COMPUTERLEVELID");
					String personcomputercert = object
							.getString("COMPUTERCERT");
					String personlanguage1 = object.getString("LANGUAGEID_1");
					String personlanguage2 = object.getString("LANGUAGEID_2");
					String personchendu1 = object
							.getString("LANGUAGEPROFICIENCYID_1");
					String personchendu2 = object
							.getString("LANGUAGEPROFICIENCYID_2");
					String personwaiyuzhengshu = object
							.getString("LANGUAGECERT");
					String personotherzhengshu = object.getString("OTHERCERTS");
					String personselfpingjia = object
							.getString("SELFEVALUATION");
	...
	public static String postMarJson(ArrayList<PersonMark> list_mark) {
		...
		for (PersonMark personmark : list_mark) {
			JSONObject jsonObject = new JSONObject();
			try {
				if (personmark.getPersonMarkId() != null
						&& personmark.getPersonMarkId().equals("-1")) {
					jsonObject.put("ID", "-1");
					...
					jsonObject.put("CREATE_DATE",
							personmark.getPersonMarkCreatdate());
					jsonObject.put("SFZ", personmark.getPersonSFZ());
					jsonObject.put("GPS", FirstPageActivity.GetGpsInfo());
				}
		...
		Log.e("2017-9-13","上传标识类型=="+jsonarray);
