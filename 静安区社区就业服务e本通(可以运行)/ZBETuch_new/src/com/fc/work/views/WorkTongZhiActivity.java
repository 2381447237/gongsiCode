...
import com.fc.work.bean.WorkTongZhiAdapter;
...
public class WorkTongZhiActivity extends Activity implements IActivity,
		OnPullDownListener {
	private TextView totalWorkNum;
	private PullDownView workPullDownView;
	...
	public void refresh(Object... params) {
		...
		case WorkTongZhiActivity.REFRESH_WORKINFO:
			if (!"".equals(params[1].toString().trim())) {
				List<WorkTongzhiBean> newList = parseJsonToList(params[1]
						.toString().trim());
				if (newList.size() > 0) {
	...
	private void initView() {
		...
		workPullDownView = (PullDownView) this
				.findViewById(R.id.mWorkPullDownView);
	...
	private void getPageList(int index) {
		...
		PersonTask task = new PersonTask(
				PersonTask.WORKTONGZHIINFOACTIVITY_GET_WORKTONGZHILIST, params);
	...
	private class MyOnItemClickListener implements OnItemClickListener {
		...
		public void onItemClick(AdapterView<?> arg0, View arg1, int position,
				long arg3) {
			// TODO Auto-generated method stub
			Intent intent = new Intent(WorkTongZhiActivity.this,
					WorkTongzhiDetail.class);
			intent.putExtra("WorkTongzhiBean",
					(Serializable) workList.get(position - 1));
	...
	private void getReadNum() {
		...
		params.put("data", data);
		PersonTask task = new PersonTask(
				PersonTask.WORKTONGZHIINFOACTIVITYDETAIL_GET_READNUM, params);
		PersonService.newTask(task);
	}
	...
	private void parseJosoToNum(String str) {
		...
		try {
			...
			totalWorkNum.setText("本月" + totalNum + "条已读" + weiduNum + "条"
					+ "读取率:" + rate + "%");
		} catch (JSONException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
