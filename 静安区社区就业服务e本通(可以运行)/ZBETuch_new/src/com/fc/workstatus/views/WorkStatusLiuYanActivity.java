...
public class WorkStatusLiuYanActivity extends Activity implements IActivity {
	...
	public void refresh(Object... params) {
		...
		case WorkStatusLiuYanActivity.WORK_STATUS_LIUYAN:
			if (params[1].toString().trim() != null) {
				List<WorkStatusMsgInfo> newList = parseJsonToBean(params[1]
						.toString().trim());
				if (newList.size() > 0) {
					workStatusMsgInfos.clear();
					workStatusMsgInfos.addAll(newList);
					adapter = new WorkStatusMsgAdapter(
							WorkStatusLiuYanActivity.this, workStatusMsgInfos);
		...
		case WorkStatusLiuYanActivity.WORK_STATUS_LIUYAN1:
			if (params[1].toString().trim() != null) {
				if ("True".equals(params[1].toString().trim())) {
					Toast.makeText(WorkStatusLiuYanActivity.this, "发表成功",
							Toast.LENGTH_SHORT).show();
					tv_msgboard_workstatus_detilecallback_doc.setText("");
					getPageList();
				} else {
					Toast.makeText(WorkStatusLiuYanActivity.this,
							"发表失败" + params[1].toString().trim(),
							Toast.LENGTH_SHORT).show();
				}
			}
			break;
	...
	private void initView() {
		...
		lsview_msg_workstatus_detitle = (ListView) this
				.findViewById(R.id.lsview_msg_workstatus_detitle);
		tv_msgboard_workstatus_detilecallback_doc = (TextView) this
				.findViewById(R.id.tv_msgboard_workstatus_detilecallback_doc);
		btn_msgboard_workstatus_detitle_callback = (Button) this
				.findViewById(R.id.btn_msgboard_workstatus_detitle_callback);

		workstatus_title.setText(title + "-留言板");
		tv_msgboard_workstatus_detilecallback_doc = (TextView) this
				.findViewById(R.id.tv_msgboard_workstatus_detilecallback_doc);
		adapter = new WorkStatusMsgAdapter(WorkStatusLiuYanActivity.this,
				workStatusMsgInfos);
		lsview_msg_workstatus_detitle.setAdapter(adapter);
		btn_msgboard_workstatus_detitle_callback
				.setOnClickListener(new View.OnClickListener() {

					@Override
					public void onClick(View v) {
						// TODO Auto-generated method stub
						if ("".equals(tv_msgboard_workstatus_detilecallback_doc
								.getText().toString().trim())
								|| tv_msgboard_workstatus_detilecallback_doc
										.getText().toString().trim() == null) {
							Toast.makeText(WorkStatusLiuYanActivity.this,
									"请输入留言内容", Toast.LENGTH_LONG).show();
							return;
						}
						// 保存留言留言信息
						...
						PersonTask task = new PersonTask(
								PersonTask.WORKSTATUSDEATIL_SET_WORK_NEWS,
								params);
	...
	private void getPageList() {
		...
		params.put("data", data);
		PersonTask task = new PersonTask(
				PersonTask.WORKSTATUSDEATIL_GET_WORK_NEWS, params);
		PersonService.newTask(task);
	}
	...
	private class MyOnItemClickListener implements OnItemClickListener {
		...
		public void onItemClick(AdapterView<?> arg0, View arg1, int position,
				long arg3) {
			// TODO Auto-generated method stub
			Intent intent = new Intent(WorkStatusLiuYanActivity.this,
					WorkStatusLiuYanDetailActivity.class);
			intent.putExtra("Master_id", workStatusMsgInfos.get(position - 1)
					.getID());
