...
public class MeetingListActivity extends Activity implements IActivity,
		OnPullDownListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		// CompanyTask(CompanyTask.MEETINGPOST_MEETINGLISTACTIVITY_GET_METTING_READ,
		// params);
	...
	private void initView() {
		...
		tv_totalandread = (TextView) findViewById(R.id.tv_totalandread);
		mPullDownView = (PullDownView) findViewById(R.id.pdvShow);
	...
	private class myOnClickListener implements OnClickListener {
		...
		public void onClick(View v) {
			...
			case R.id.ibtn_now:
				state = "true";
				ibtn_now.setImageDrawable(getResources().getDrawable(
						R.drawable.meetnow_white));
				ibtn_history.setImageDrawable(getResources().getDrawable(
						R.drawable.meethistory_gery));
				getPageList(index, state);
				break;
			case R.id.ibtn_history:
				state = "false";
				ibtn_history.setImageDrawable(getResources().getDrawable(
						R.drawable.meethistory_white));
				ibtn_now.setImageDrawable(getResources().getDrawable(
						R.drawable.meetnow_gery));
				getPageList(index, state);
				break;
			default:
				break;
		...
		case MeetingListActivity.GET_MEETING_READ:
			if (params[1].toString().trim() != null
				...
				if (newlist.size() > 0 && newlist != null) {
					...
					tv_totalandread.setText("本月"
							+ meetingreadInfos.get(0).getValue1() + "条,已读"
							+ meetingreadInfos.get(0).getValue2() + "条"
							+ "读取率:" + meetingreadInfos.get(0).getValue3()
							+ "%");
				} else {
					tv_totalandread.setText("本月" + 0 + "条,已读" + 0 + "条  "
							+ "读取率:" + 0.0 + "%");
				}
				break;
			}
	...
	private void getPageList(int index, String State) {
		...
		CompanyTask task = new CompanyTask(
				CompanyTask.MEETINGPOST_MEETINGLISTACTIVITY_GET_MEETING_MASTER,
				params);
		CompanyService.newTask(task);
	}
	...
	private void getReadNum() {
		...
		params.put("data", data);
		CompanyTask task = new CompanyTask(
				CompanyTask.MEETINGPOST_MEETINGLISTACTIVITY_GET_METTING_READ,
				params);
		CompanyService.newTask(task);
	}
