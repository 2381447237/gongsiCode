...
public class MeetingListDetailActivity extends Activity implements IActivity {
	...
	private TextView tv_meetname, tv_meettime, tv_meetadress, tv_meetdoc,
			tv_meetpostperson, tv_meetposttime, tv_meetaddperson;
	...
	private void initView() {
		...
		tv_meettime.setText(info.getMeetingTime().replace("T", " ")
				.substring(0, 19));
		...
		tv_meetposttime.setText(info.getCreateDate().replace("T", " ")
				.substring(0, 19));
	...
	private void getButtonUserful() {
		...
		CompanyTask task = new CompanyTask(
				CompanyTask.MEETINGPOST_MEETINGLISTDETAILACTIVITY_GET_METTING_CHECK,
				params);
	...
	public void btnCheckMeetFair_OnClick(View view) {
		...
		params.put("data", data);
		CompanyTask task = new CompanyTask(
				CompanyTask.MEETINGPOST_MEETINGLISTDETAILACTIVITY_SET_MEETING_CHECK,
				params);
		CompanyService.newTask(task);
	}
	...
	private class MyOnItemClickListener implements OnItemClickListener {
		...
		public void onItemClick(AdapterView<?> arg0, View arg1, int position,
				long arg3) {
			String filePath = ("/" + ((MeetingDownloadInfo) adapter
					.getItem(position)).getFilePath().replaceAll(" ", "%20"));
			String[] str = filePath.split("/");
			currentFile = str[str.length - 1];
			Map<String, Object> params = new HashMap<String, Object>();
			params.put("path", filePath);
			CompanyTask task = new CompanyTask(
					CompanyTask.MEETINGPOST_MEETINGLISTDETAILACTIVITY_GET_METTING_DOWNLOADITEM,
					params);
			CompanyService.newTask(task);
			showDialog();
		}
	...
	private void getFileList() {
		...
		params.put("data", data);
		CompanyTask task = new CompanyTask(
				CompanyTask.MEETINGPOST_MEETINGLISTDETAILACTIVITY_GET_METTING_FILE,
				params);
		CompanyService.newTask(task);
	}
