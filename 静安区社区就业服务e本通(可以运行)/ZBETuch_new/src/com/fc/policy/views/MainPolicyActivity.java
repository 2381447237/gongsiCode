...
public class MainPolicyActivity extends Activity implements IActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		MainTask task = new MainTask(
				MainTask.MAINPOLICYACTIVITY_GET_POLICYTYPE, null);
	...
	public void refresh(Object... params) {
		...
		case MainPolicyActivity.REFRESH_CBOTYPE:
			if (params[1] != null) {
				...
				MainTools.fetchSpinner(this, cboPolicyStyle, cboTypeStr, "ID",
						"TYPE_NAME");
				if(!TextUtils.isEmpty(cboTypeStr)){		
	...
	private class MyOnclickListener implements OnClickListener {
		...
		public void onClick(View v) {
			...
			case R.id.btnquery:
				...
				String type = "";
				if (!((SpinnerItem) cboPolicyStyle.getSelectedItem()).getName()
						.trim().equals("请选择")) {
					type = ((SpinnerItem) cboPolicyStyle.getSelectedItem())
							.getName().trim();
				}
				params.put("type", type);
				params.put("keyWord", txtPolicyKeywords.getText().toString()
						.trim());
				CompanyTask task = new CompanyTask(
						CompanyTask.MAINPOLICYACTIVITY_GET_POLICY, params);
				CompanyService.newTask(task);
				// }
				break;
			case R.id.btnask:
				Intent intent = new Intent(MainPolicyActivity.this,
						PolicyAskActivity.class);
	...
	private void fretchLv(String policyStr) {
		...
		// answers, R.layout.activity_policy_main_subitem, new
		// String[]{"answer"}, new int[]{R.id.txtSubItem});
