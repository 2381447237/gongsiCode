...
public class ZhiYuanDetailListActivity extends Activity implements IActivity,
		OnPullDownListener {
	...
	public void refresh(Object... params) {
		...
		case REFRESH_INFO:
			...
			if (!"".equals(params[1].toString().trim())
				...
				List<ResourcesDetailInfo> newInfos = parseJsonToList(params[1]
						.toString().trim());
				if (newInfos.size() > 0) {
				...
				if (detailInfos.size() > 0) {
					lblNum.setText("总共" + detailInfos.get(0).getRecordCount()
							+ "条数据");
				} else {
	...
	private void initView() {
		...
		rb1 = (RadioButton) this.findViewById(R.id.btn_weicha);
		rb2 = (RadioButton) this.findViewById(R.id.btn_yicha);
		...
		detailAdapter = new ResourcesDetailAdapter(
				ZhiYuanDetailListActivity.this, detailInfos);
	...
	private class MyOnitemClickListener implements OnItemClickListener {
		...
		public void onItemClick(AdapterView<?> arg0, View arg1, int position,
			...
			if ("无业".equals(resourcesInfo.getTYPE())) {
				intent.setClass(ZhiYuanDetailListActivity.this,
						WuYeDetailActivity.class);
			} else if ("失业".equals(resourcesInfo.getTYPE())) {
				intent.setClass(ZhiYuanDetailListActivity.this,
						ShiYeDetailActivity.class);
			}
			if (rb1.isChecked()) {
			...
			intent.putExtra("infos",
					(Serializable) detailInfos.get(position - 1));
	...
	private void getPageList(int index) {
		...
		PersonTask task = new PersonTask(
				PersonTask.ZHIYUANDETAILACTIVITY_GET_INFOS, params);
	...
	private class MyOnTouchListener implements OnTouchListener {
		...
		public boolean onTouch(View v, MotionEvent event) {
			...
			case R.id.shiye_IdCard:
				...
				// showPopupWindow(v);
				popupwindow = MainTools
						.showPopupWindow(ZhiYuanDetailListActivity.this,
								popupwindow, idcardText);
	...
	private class MyOnClickListener implements OnClickListener {
		...
		public void onClick(View v) {
			...
			if ("".equals(idcardText.getText().toString().trim())
				...
				Toast.makeText(ZhiYuanDetailListActivity.this, "身份证号不能为空",
						Toast.LENGTH_LONG).show();
				return;
			}
			if (idcardText.getText().toString().trim().length() != 18) {
				Toast.makeText(ZhiYuanDetailListActivity.this, "请输入18位身份证号",
						Toast.LENGTH_LONG).show();
				return;
			}
			Map<String, Object> params = new HashMap<String, Object>();
			...
			params.put("data", data);
			PersonTask task = new PersonTask(
					PersonTask.ZHIYUANDETAILACTIVITY_GET_SFZ_IFNO, params);
			PersonService.newTask(task);
		}
