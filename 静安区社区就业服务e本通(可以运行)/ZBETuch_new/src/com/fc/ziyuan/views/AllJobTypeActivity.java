...
public class AllJobTypeActivity extends Activity implements IActivity {
	...
	private OnClickListener btnclick = new OnClickListener() {
		...
		public void onClick(View v) {
			...
			case R.id.btnok:
				...
				if (j > 3) {
					Toast.makeText(AllJobTypeActivity.this, "最多选3项", 1).show();
				} else {
					if (boxs != null && boxs.size() > 0) {
						for (int i = 0; i < boxs.size(); i++) {
							if (boxs.get(i).isChecked()) {
					...
					intent.putExtra("jobtype", (Serializable) name);
					setResult(101, intent);
					AllJobTypeActivity.this.finish();
				}
				break;
			case R.id.btncancle:
				AllJobTypeActivity.this.finish();
	...
	private void initView() {
		...
		jobtx.setText("希望从事工作类型（最多选3项）");
		jobgv = (GridView) findViewById(R.id.jobgv);

		btn_ok = (Button) findViewById(R.id.btn_Ok);
		btn_cancle = (Button) findViewById(R.id.btn_Cancle);
		all = getResources().getStringArray(R.array.jobtype);
		...
		for (String name : all) {
			CheckBox box = (CheckBox) getLayoutInflater().from(
					AllJobTypeActivity.this).inflate(
