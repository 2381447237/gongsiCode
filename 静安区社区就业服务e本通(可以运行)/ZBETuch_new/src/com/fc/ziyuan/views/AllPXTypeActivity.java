...
public class AllPXTypeActivity extends Activity implements IActivity {
	...
	private OnClickListener btnclick = new OnClickListener() {
		...
		public void onClick(View v) {
			...
			case R.id.btnok:
				...
				if (j > 3) {
					Toast.makeText(AllPXTypeActivity.this, "最多选3项", 1).show();
				} else {
					if (boxs != null && boxs.size() > 0) {
						for (int i = 0; i < boxs.size(); i++) {
							if (boxs.get(i).isChecked()) {
					...
					intent.putExtra("type", (Serializable) name);
					setResult(102, intent);
					AllPXTypeActivity.this.finish();
				}
				break;
			case R.id.btncancle:
				AllPXTypeActivity.this.finish();
	...
	private void initView() {
		...
		jobtx.setText("希望接受职业培训类别（最多选3项）");
		btn_ok = (Button) findViewById(R.id.btn_Ok);
		btn_cancle = (Button) findViewById(R.id.btn_Cancle);
		all = getResources().getStringArray(R.array.traintype);
		...
		for (String name : all) {
			CheckBox box = (CheckBox) getLayoutInflater().from(
					AllPXTypeActivity.this).inflate(
