	...
	public void refresh(Object... params) {
		...
		case ResourcesMainActivity.REFRESH_LVRESOURCES:
			...
			if (params[1] != null) {
				...
				try {
					if(resourcesListStr!=null && resourcesListStr.length()>0){
		...
		if(items!=null && items.size()>0){
			...
			for(ResourcesItem item : items){
				...
				if(!orderidStr.trim().substring(0, 1).equals("4")){
					groupItems.add(item);
					itemMap.put(item.getCommittee_id().trim()+item.getType().trim(),index );
					index++;
				}
			}
		}		
	...
	private void fretchSubItems(List<ResourcesItem> items){
		if(items!=null && items.size()>0){
			...
			for(ResourcesItem item : items){
				String orderidStr = ""+item.getOrder_id();
				if(orderidStr.trim().substring(0, 1).equals("4")){
					subItems.get(itemMap.get(orderidStr.trim().substring(1)+item.getType().trim())).add(item);					
				}
			}
		}		
	}
...
//				showPopupWindow(v);
				popupwindow = MainTools.showPopupWindow(mContext, popupwindow, txtIdentity);
	...
	private class MyOnItemClickListener implements OnItemClickListener{
		...
		public void onItemClick(AdapterView<?> arg0, View arg1, int location,
			...
			Intent intent = new Intent(ResourcesMainActivity.this,PersonqueryListActivity.class);
			intent.putExtra("flag", PersonqueryListActivity.FLAG_FROM_RESOURCES);
