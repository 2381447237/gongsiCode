		...
		public void onClick(View v) {
			...
			case R.id.btnQueryById:
				if (txtIdCard.getText().toString().trim().length()==18||txtIdCard.getText().toString().trim().length()==0) {
					Intent intent2 = new Intent(InsertIdCardActivity.this,PersonPolicyMainActivity.class);
					intent2.putExtra("mysfz", txtIdCard.getText().toString().trim());
					intent2.putExtra("code", txtCode.getText().toString().trim());
					intent2.putExtra("type", type);
					startActivity(intent2);
				}else{
					Toast.makeText(mContext, "身份证号必须为18位,请重新输入......", Toast.LENGTH_SHORT).show();
					return ;
				}
				break;
