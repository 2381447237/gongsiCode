...
public class ModifyStatusMarkActivity extends Activity implements IActivity{
	...
	public Button btn_modify_mark,button_addmark,button_markok,button_markno;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		PersonTask task=new PersonTask(PersonTask.MODIFYPERSONMARK_GET_PERSONINFOS, params);
	...
	public void refresh(Object... params) {
		...
		case REFRESH_INFO:
			if (params[1].toString().trim()!=null&&!params[1].toString().trim().equals("")) {
		...
		case REFRESH_TYPE_INFO:
			if (!"".equals(params[1].toString().trim())&&params[1].toString().trim()!=null) {
				MainTools.fetchSpinner(ModifyStatusMarkActivity.this, spinner_personmark, params[1].toString().trim(), "ID", "NAME");
			}
			break;
		...
		case REFRESH_TYPE_INFO2:
			if (!"".equals(params[1].toString().trim())&&params[1].toString().trim()!=null) {
			    if ("True".equals(params[1].toString().trim())) {
					Toast.makeText(ModifyStatusMarkActivity.this, "操作成功!", Toast.LENGTH_SHORT).show();
					getTypeInfo();
				} else {
					Toast.makeText(ModifyStatusMarkActivity.this, "操作失败!", Toast.LENGTH_SHORT).show();
				}
			}
			break;
		...
		case UPLOAD_INFO:
			if (!"".equals(params[1].toString().trim())&&params[1].toString().trim()!=null) {
				if ("True".equals(params[1].toString().trim())) {
					Toast.makeText(ModifyStatusMarkActivity.this, "上传成功", Toast.LENGTH_SHORT).show();
				} else {
					Toast.makeText(ModifyStatusMarkActivity.this, "上传失败", Toast.LENGTH_SHORT).show();
				}
			}
			break;
	...
	private void initView(){
		...
		spinner_personmark=(Spinner) this.findViewById(R.id.spinner_personmark);
	...
	private class MyOnClickListener implements OnClickListener{
		...
		public void onClick(View v) {
			...
			case R.id.button_mark:
				Intent intent=new Intent(ModifyStatusMarkActivity.this,ModifyStatusPersonMarkActivity.class);
	...
	private void getTypeInfo(){
		...
		PersonTask task1=new PersonTask(PersonTask.MODIFYPERSONMARK_GET_TYPE_INFO, params1);
		...
		dialog.setTitle(title).setMessage(msg)
				...
				.setPositiveButton("确定", new DialogInterface.OnClickListener() {
					...
					public void onClick(DialogInterface dialog, int which) {
						...
						params.put("data", data);
						PersonTask task=new PersonTask(PersonTask.MODIFYPERSONMARK_INFO, params);
						PersonService.newTask(task);
 					}
				})
