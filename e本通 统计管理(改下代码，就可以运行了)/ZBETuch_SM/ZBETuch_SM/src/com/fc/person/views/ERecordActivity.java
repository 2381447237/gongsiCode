...
import com.fc.recritmentmanager.bean.ERecordAdapter;
import com.fc.recritmentmanager.bean.ERecordInfo;
...
public class ERecordActivity extends Activity implements IActivity ,OnPullDownListener{
	private PullDownView2 myPullDownView;
	private ListView myListView;
	private List<ERecordInfo> eRecordInfos=new ArrayList<ERecordInfo>();
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_person_erecord);
		init();
		initView();
		initPullDownView();
		getPageList(index);
	...
	public void refresh(Object... params) {
		...
		case ERECORD_INFO:
			if (!"".equals(params[1].toString().trim())||null!=params[1].toString().trim()) {
				List<ERecordInfo> newInfos=parseJsonToInfo(params[1].toString().trim());
				if (newInfos.size()>0) {
					eRecordInfos.addAll(newInfos);
				}
	...
	private void initView(){
		...
		adapter=new ERecordAdapter(eRecordInfos, ERecordActivity.this);
	...
	private void getPageList(int index){
		...
		CompanyTask task=new CompanyTask(CompanyTask.ERECORD_ERECORDACTIVITY_GET_ERECORDINFO , params);
		CompanyService.newTask(task);
	}
	
	private List<ERecordInfo> parseJsonToInfo(String str){
		List<ERecordInfo> infos=new ArrayList<ERecordInfo>();
		try {
			JSONArray jsonArray=new JSONArray(str);
			for (int i = 0; i < jsonArray.length(); i++) {
				JSONObject jsonObject=(JSONObject) jsonArray.get(i);
				ERecordInfo info=new ERecordInfo();
