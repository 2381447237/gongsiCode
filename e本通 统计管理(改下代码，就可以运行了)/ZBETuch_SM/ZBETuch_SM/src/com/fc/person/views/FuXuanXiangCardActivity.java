...
public class FuXuanXiangCardActivity extends Activity implements IActivity{
	...
	private Button newButton,refresh_fuwu;
			...
			case FuXuanXiangCardActivity.DELETE_FUWU:
				if (params[1] != null) {
					...
					if (value.trim().equalsIgnoreCase("true")) {
						...
						Toast.makeText(mContent, "删除失败！"+value, Toast.LENGTH_SHORT)
						.show();
					}
				}
				break;
	...
	private void getAllFuXuan(){
		...
		PersonTask task = new PersonTask(
				PersonTask.FUXUANGXIANGKA_GET_INFO, params);
	...
	private class MyOnClickListener implements OnClickListener{
		...
		public void onClick(View v) {
			// TODO Auto-generated method stub
			Intent intent=new Intent(mContent.getParent(),NewFuWuXuanXiangActivity.class);
			intent.putExtra("SFZ", my_sfz);
			if (fuXuanXiangBeans.size()>0&&fuXuanXiangBeans!=null) {
				intent.putExtra("lastFuWuXuanXiang", fuXuanXiangBeans.get(fuXuanXiangBeans.size()-1));
			}
