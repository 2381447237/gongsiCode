...
public class PersoninfoActivity extends Activity implements IActivity {
	...
	public void refresh(Object... params) {
		...
		case PersoninfoActivity.REFRESH_REGION_SPINNER:
			String regionStr = "";
			if (params[1] != null) {
			...
			break;
		case PersoninfoActivity.REFRESH_STREET_SPINNER:
			String streetStr = "";
			if (params[1] != null) {
			...
			break;
		case PersoninfoActivity.REFRESH_JUWEIHUI_SPINNER:
			String juweihuiStr = "";
			if (params[1] != null) {
		...
		public void onClick(View v) {
			...
			if( v.getId() == R.id.bt_query)
			{
				...
				if (personSFZ.equals("")) {
					...
					Intent ientent = new Intent(PersoninfoActivity.this,
			...
			else if( v.getId() == R.id.bt_query2)
			{
				// 判断年龄段 及传入参数
				...
				if (!ageLow.equals("")
					...
					String personName = et_personName.getText().toString()
							.trim();
					Intent intentquery2 = new Intent(PersoninfoActivity.this,
					...
					intentquery2.putExtra("flag",
							PersonqueryListActivity.FLAG_FROM_PERSONINFO);
					startActivityForResult(intentquery2, 200);
					// startActivity(intentquery2);
				}
			}
			else if( v.getId() ==  R.id.bt_scanning)
			{
				// 扫描
				Intent intent = new Intent();
				intent.setClass(PersoninfoActivity.this, CaptureActivity.class);
	...
	private class MySpinnerItemSeletedListener implements
		...
		public void onItemSelected(AdapterView<?> arg0, View view, int arg2,
			...
			else if(arg0.getId() == R.id.spinner_street)
			{
			...
			else if(arg0.getId() == R.id.spinner_type)
			{
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if (requestCode == 100) {// 扫描返回的结果
			...
			if (resultCode == RESULT_OK) {
				Toast.makeText(PersoninfoActivity.this, "对不起，您的操作超时，请重新登录！",
			...
			if (resultCode == RESULT_OK) {
				...
				if (personbaseinfo != null && personbaseinfo.size() == 0) {
					Toast.makeText(PersoninfoActivity.this, "对不起，查无此人！",
							Toast.LENGTH_SHORT).show();
				}
			}
