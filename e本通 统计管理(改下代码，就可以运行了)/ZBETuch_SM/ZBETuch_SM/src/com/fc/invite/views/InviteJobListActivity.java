...
public class InviteJobListActivity extends Activity implements IActivity,
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		case InviteJobListActivity.GET_ITEMS_BY_PARAMS:
			...
			data.put("ComPropertyId", ""+intent.getIntExtra("ComPropertyId", -1));
			...
			data.put("IndustryClassId", ""+intent.getIntExtra("IndustryClassId", -1));
			data.put("IndustryClassChildId", ""+intent.getIntExtra("IndustryClassChildId", -1));
			...
			data.put("ModifyStartDate", intent.getStringExtra("ModifyStartDate"));
			data.put("ModifyEndDate", intent.getStringExtra("ModifyEndDate"));
			data.put("IsDirectInterview", ""+intent.getBooleanExtra("IsDirectInterview", false));
			data.put("IsNewGraduates", ""+intent.getBooleanExtra("IsNewGraduates", false));
			data.put("IsDisabledPerson", ""+intent.getBooleanExtra("IsDisabledPerson", false));
			data.put("IsAssurance", ""+intent.getBooleanExtra("IsAssurance", false));
		...
		case InviteJobListActivity.GETMOREITEM:
			if(params[1]!=null){
				...
				if(newJobs!=null && newJobs.size()>0){
					...
					lblAllNum.setText("共有数据"+newJobs.get(0).getMax_row()+"条");
				}
				if(jobList.size()==0){
	...
	private void getPageList(int index) {
		...
		if(currentSty==InviteJobListActivity.GET_ITEMS_ALL){
			...
			CompanyTask task = new CompanyTask(CompanyTask.INVITEJOBLIST_BYPAGE,
					params);
			CompanyService.newTask(task);
		}else if(currentSty== InviteJobListActivity.GET_ITEMS_BY_COMPANY){
			if(data!=null){
				params.put("data", data);
				CompanyTask task = new CompanyTask(CompanyTask.INVITEJOBLISTACTIVITY_GET_JOBLIST_BY_COMPANY, params);
				CompanyService.newTask(task);
			}
			...
			if(data!=null){
				...
				params.put("data", data);
				CompanyTask task = new CompanyTask(CompanyTask.INVITEJOBLISTACTIVITY_GET_JOBLIST_BY_PARAMS, params);
				CompanyService.newTask(task);
			}
			...
			if(data!=null){
				...
				data.put("PageRecCnts", ""+15);
				params.put("data", data);
				CompanyTask task = new CompanyTask(CompanyTask.INVITEJOBLISTACTIVITY_GET_JOBLIST_BY_SFZ, params);
				CompanyService.newTask(task);
			}
		}
	}
	...
	private class MyOnItemClickListener implements OnItemClickListener {
		...
		public void onItemClick(AdapterView<?> arg0, View arg1, int position,
			...
			Intent intent = new Intent(InviteJobListActivity.this,InviteJobDetailActivity.class);			
