...
public class RedecodeAdapter extends BaseAdapter{
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		// TODO Auto-generated method stub
				...
				if (convertView==null) {
					convertView=LayoutInflater.from(mContext).inflate(R.layout.item_redecode, null);
		            ...
		            holderItem.sfz_TextView=(TextView) convertView.findViewById(R.id.tv_sfz);
				...
				holderItem.type_TextView.setText(info.getCREATE_TIME().replace("T", " ").substring(0, 10));
				if (info.getGPS().indexOf(",")>0&&info.getGPS().trim()!=null&&!"".equals(info.getGPS().trim())) {
					String gps[] =info.getGPS().split(",");
					if (gps[0].indexOf(".")>0&&gps[1].indexOf(".")>0) {
						holderItem.time_TextView.setText("经度:"+gps[0].substring(0, gps[0].indexOf("."))+"\n"+"纬度:"+gps[1].substring(0, gps[1].indexOf(".")));
					} else {
			...
			class HolderItem{
				TextView num_TextView,name_TextView,type_TextView,time_TextView,sfz_TextView;
			}
