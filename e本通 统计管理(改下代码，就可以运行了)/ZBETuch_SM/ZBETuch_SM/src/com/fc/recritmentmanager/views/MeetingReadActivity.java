...
import com.fc.recritmentmanager.bean.MeetingReadAdapter;
...
public class MeetingReadActivity extends Activity implements IActivity{
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		CompanyTask task=new CompanyTask(CompanyTask.MEETING_READ_GROUP, params);
	...
	public void refresh(Object... params) {
		...
		case REFRESH_INFO:
			if (!"".equals(params[1].toString().trim())&&params[1].toString().trim()!=null) {
				List<ReadInfo> newInfos=parseJsonToMeetReadInfo(params[1].toString().trim());
				if (newInfos.size()>0) {
