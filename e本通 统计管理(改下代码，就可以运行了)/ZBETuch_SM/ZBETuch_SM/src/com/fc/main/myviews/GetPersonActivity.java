...
public class GetPersonActivity extends Activity implements IActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		onActivityResult(100,100,getIntent());
	...
	public void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if (requestCode==100&&resultCode==100) {
			List<PersonItem> newPersonItems=(List<PersonItem>) data.getSerializableExtra("persons");
				...
				if (allPersons.size()>0&&allPersons!=null) {
					for (int i = 0; i < allPersons.size(); i++) {
						for (int j = 0; j < newPersonItems.size(); j++) {
							if (allPersons.get(i).getStaff_id()==newPersonItems.get(j).getStaff_id()) {
...
//		if (resultCode==10){
//			allPersonsBack = (List<PersonItem>) data.getSerializableExtra("persons");
//			adapter=new PersonItemsAdapter(allPersonsBack,GetPersonActivity.this);
//			allPeople.setAdapter(adapter);
//		}
	}
	...
	private void fretchTree(LinearLayout fatherLayout, final LineLevelItem item) {
		...
		lblName.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				Intent intent=new Intent(GetPersonActivity.this, AllPeopleActivity.class);
				if ("search".equals(getIntent().getAction())) {
					intent.setAction("search");
				} else{
					intent.setAction("slse");
				}
				intent.putExtra("id", ""+item.getId());
				startActivityForResult(intent, 100);
//				v.setBackgroundColor(0x7f070023);
	...
	private class MyOnClickListener implements OnClickListener {
		...
		public void onClick(View v) {
			switch (v.getId()) {
			case R.id.btnOk:
				Intent intent = new Intent(GetPersonActivity.this,SearchPendWorkActivity.class);
