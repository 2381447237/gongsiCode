...
public class GradeateParamQueryActivity extends Activity implements IActivity {
	...
	public void refresh(Object... params) {
		...
		case GradeateParamQueryActivity.REFRESH_COMPROPERTY:
			if(params[1]!=null){
				...
				if(value.length()>0){
					MainTools.fetchSpinner(mContext, sp_qiye, value,"compropertyid","compropertyname");
				}
			}
			break;
		case GradeateParamQueryActivity.REFRESH_INDUSTRYCATAGORY:
			if(params[1]!=null){
				String value = params[1].toString().trim();
				if(value.length()>0){
					MainTools.fetchSpinner(mContext, sp_hangye, value,"id","name");
				}
			}
			break;
	...
	private void initSpinner() {
		...
		CompanyTask task3 = new CompanyTask(CompanyTask.GRADEATE_GradeateParamQueryActivity_GET_STREETLIST, params);
	...
	public void init() {
		...
		CompanyTask task1 = new CompanyTask(CompanyTask.GRADEATE_GradeateParamQueryActivity_GET_GANGWEILEIBIE,null);
		CompanyService.newTask(task1);
		CompanyTask task2 = new CompanyTask(CompanyTask.GRADEATE_GradeateParamQueryActivity_GET_HANGYELEIBIE,null);
	...
	private class MyOnClickListener implements OnClickListener{
		...
		public void onClick(View v) {
			...
			case R.id.btn_query:
				...
				data.put("STREET_ID", ((StreetInformation)sp_jiedao.getSelectedItem()).getStreetId());
				data.put("COMMITTEE_ID",((CommitteeInformation) sp_juwei.getSelectedItem()).getCommitteeId());
				...
				data.put("COMPROPERTY_ID",""+((SpinnerItem)sp_qiye.getSelectedItem()).getId());
				data.put("INDUSTARY_CATEGORY_ID",""+((SpinnerItem) sp_hangye.getSelectedItem()).getId());
				...
				}else if(sp_zhidao.getSelectedItem().toString().trim().equals("否")){
				...
				}else if(sp_jiuye.getSelectedItem().toString().trim().equals("否")){
					data.put("CHECK_RECOMMEND","false");
				}else{
					data.put("CHECK_RECOMMEND", "");
				}
				data.put("year", getIntent().getStringExtra("year"));
				IActivity activity= (IActivity) CompanyService.getActivityByName("GradeateListActivity");
				if(activity!=null){
					activity.refresh(GradeateListActivity.REFRESH_BY_PARAM,data);
				}
				mContext.finish();
			}
		}
	 ...
	 * @param temp 存取的map对象
	 * @param idName map的键值
	 * @param spName spinner的id
	 */
	private void dataSelectSpinner(Map<String, String> temp,String idName,Spinner spName){
	 ...
	 * @param temp 存取的map对象
	 * @param idName map的键值
	 * @param edName EditText的id
	 */
	private void dataSelectEditText(Map<String, String> temp,String idName,EditText edName){
