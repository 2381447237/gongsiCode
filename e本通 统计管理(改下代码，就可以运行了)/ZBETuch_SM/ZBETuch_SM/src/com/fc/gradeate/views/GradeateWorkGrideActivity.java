...
public class GradeateWorkGrideActivity extends Activity implements IActivity,OnPullDownListener{
				...
				if (infos.size()>0) {
					for (int i = 0; i < infos.size(); i++) {
						...
						if (jobGrideInfo.getId()==info.getId()) {
							...
							jobGrideInfo.setCheck_recommend(info.getCheck_recommend());
							...
							jobGrideInfo.setRecommend_com(info.getRecommend_com());
							jobGrideInfo.setRecommend_date(info.getRecommend_date());
							jobGrideInfo.setRecommend_job(info.getRecommend_job());
							jobGrideInfo.setRecordCount(info.getRecordCount());
							jobGrideInfo.setUpdate_date(info.getUpdate_date());
							jobGrideInfo.setUpdate_staff(info.getUpdate_staff());
						}
					}
				}
			}
	...
	private void initView() {
		...
		mPullDownView = (PullDownView2) findViewById(R.id.lvJobGride);
			...
			try {
				...
				if (jsonArray != null && jsonArray.length() > 0) {
					for (int i = 0; i < jsonArray.length(); i++) {
						...
						info.setRecommend_date(object.getString("RECOMMEND_DATE").trim());
