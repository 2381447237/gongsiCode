...
OnPullDownListener {
	...
	private PullDownView2 pdvGradeate;
		...
		case GradeateListActivity.REFRESH_GRADEATELIST:
			if (params[1] != null) {
				...
				if(gradeateList!=null && gradeateList.size()>0){
					lblNum.setText("共有"+gradeateList.get(0).getRecordCount()+"人");
				}else {
					lblNum.setText("共有0人");
			...
			if (gradeateList.size()>0) {
				for (GradeateInfo gradeateInfo : gradeateList) {
					if (gradeateInfo.getSfz().equals(infos.getSfz())) {
						...
						gradeateInfo.setPoliticalStatus(infos.getPoliticalStatus());
	...
	private void initView() {
		...
		pdvGradeate = (PullDownView2) findViewById(R.id.pdvGradeate);
	...
	private void initValue(){
		...
		ArrayAdapter<Integer> adapter=new ArrayAdapter<Integer>(mContext, android.R.layout.simple_spinner_item, list);
	...
	private class MyOnTouchListener implements OnTouchListener {
		...
		public boolean onTouch(View v, MotionEvent event) {
			...
			case R.id.txtIdCard:
				...
				//showPopupWindow(v);
				popupwindow = MainTools.showPopupWindow(mContext, popupwindow, txtIdCard);
	...
	private class MyOnClickListener implements OnClickListener {
		...
		public void onClick(View v) {
			...
			case R.id.btnQuery:
				...
				System.out.println("currrnt_time-time="+(currrnt_time-time));
				if (currrnt_time-time<2000) {
					Toast.makeText(GradeateListActivity.this, "数据查询中...", Toast.LENGTH_SHORT).show();
					time=currrnt_time;
					return ;
				}
				if (txtIdCard.getText().toString().trim().length()==18||txtIdCard.getText().toString().trim().length()==0) {
					index = 0;
					gradeateList.clear();
					adapter.notifyDataSetChanged();
					...
					getPageList(index);
						...
						Toast.makeText(mContext, "身份证号输入不正确！ 请重新输入......", Toast.LENGTH_LONG).show();
						return;
					}
			...
			case R.id.btnParamQuery:
				...
				Intent paramQueryIntent = new Intent(mContext,GradeateParamQueryActivity.class);
				paramQueryIntent.putExtra("streetStr", streetStr);
				paramQueryIntent.putExtra("year",yearSpinner.getSelectedItem().toString());
