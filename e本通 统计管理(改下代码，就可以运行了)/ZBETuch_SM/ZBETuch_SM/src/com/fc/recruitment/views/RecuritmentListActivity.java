...
OnPullDownListener{
	...
	public void init() {
		...
		mPullDownView = (PullDownView2)findViewById(R.id.lv_recuritment);
	...
	public class MyOnItemClickListener implements OnItemClickListener{
		...
		public void onItemClick(AdapterView<?> parent, View view, int position,
				long id) {
				Intent intent = new Intent(mContext,RecuritmentListDetailActivity.class);
	...
	private void getPageList(int index) {
		...
		CompanyTask task = new CompanyTask(
				CompanyTask.RECRUITMENT_RecuritmentListActivity_GET_RECRUITMENTLIST, params);
			...
			if (str.length() > 0) {
				...
				if (jsonArray != null && jsonArray.length() > 0) {
					for (int i = 0; i < jsonArray.length(); i++) {
						...
						info.setJobFairData(object.getString("JOBFAIRDATA").replace("T", " "));
						info.setAddress(object.getString("ADDRESS").trim());
						info.setCreateDate(object.getString("CREATE_DATE").trim());
