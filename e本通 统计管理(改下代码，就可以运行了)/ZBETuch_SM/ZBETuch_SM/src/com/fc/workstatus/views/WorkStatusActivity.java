...
public class WorkStatusActivity extends Activity implements IActivity,OnPullDownListener{
	private TextView tv_totalworkstatusread;
	private PullDownView2 mWorkstatusPullDownView;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		PersonTask task=new PersonTask(PersonTask.WORKSTATUSINFOACTIVITY_GET_WORKSTATUSINFO_READ, params);
	...
	public void refresh(Object... params) {
		...
		case WorkStatusActivity.WORK_STATUS_INFO:
			 if (!"".equals(params[1].toString().trim())&&params[1].toString().trim()!=null) {
				List<WorkStatusInfo> infos=parseJsonToWorkStatus(params[1].toString().trim());
				if (infos.size()>0) {
	...
	private void initView(){
		tv_totalworkstatusread=(TextView) this.findViewById(R.id.tv_totalworkstatusread);
		mWorkstatusPullDownView=(PullDownView2) this.findViewById(R.id.mWorkstatusPullDownView);
		mWorkStatusListView=mWorkstatusPullDownView.getListView();
		group=(RadioGroup) this.findViewById(R.id.my_group);
		radioButton1=(RadioButton) this.findViewById(R.id.btn_shoudao);
		radioButton2=(RadioButton) this.findViewById(R.id.btn_yifa);
		newDongtai = (ImageButton) findViewById(R.id.new_work);
		newDongtai.setOnClickListener(new OnClickListener() {
			@Override
			public void onClick(View v) {
				// TODO Auto-generated method stub
				switch (v.getId()) {
				case R.id.new_work:
					Intent newReportIntent = new Intent(WorkStatusActivity.this,NewWorkStatusActivity.class);
		...
		group.setOnCheckedChangeListener(new RadioGroup.OnCheckedChangeListener() {
			...
			public void onCheckedChanged(RadioGroup group, int checkedId) {
				...
				index=0;
				switch (checkedId) {
				case R.id.btn_shoudao:
					data=new HashMap<String, String>();
					data.put("type_name", "收到");
					break;
					
				case R.id.btn_yifa:
					data=new HashMap<String, String>();
					data.put("type_name", "已发");
					break;
				default:
					break;
				}
		...
		workStatusAdapter=new WorkStatusAdapter(workStatusInfos, WorkStatusActivity.this);
	...
	private void getPageList(int index){
		...
		data.put("rows", "15");
		params.put("data", data);
		PersonTask task=new PersonTask(PersonTask.WORKSTATUSINFOACTIVITY_GET_WORKSTATUSINFO, params);
		PersonService.newTask(task);
	}
	...
	private void parseWorkSatusNum(String str){
		...
		try {
			...
			tv_totalworkstatusread.setText("本月"+totalNum+"条已读"+weiduNum+"条"+"读取率:"+rate+"%");
	...
	private class MyItemClickListener implements OnItemClickListener{
		...
		public void onItemClick(AdapterView<?> arg0, View arg1, int position,
				long arg3) {
			// TODO Auto-generated method stub
			Intent intent=new Intent(WorkStatusActivity.this, WorkStatusDetailActivity.class);
			intent.putExtra("workstatusInfo",(Serializable) workStatusInfos.get(position-1));
