...
import com.fc.work.beans.WorkTongDownloadInfo;
...
public class WorkStatusDetailActivity extends Activity implements IActivity{
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		workStatusInfo=(WorkStatusInfo) getIntent().getSerializableExtra("workstatusInfo");
		init();
		initView();
	...
	public void refresh(Object... params) {
		...
		case WorkStatusDetailActivity.WORKSTATUSDETAIL_SET_BUTTON_STATUS:
			if (!"".equals(params[1].toString().trim())&&params[1].toString().trim()!=null) {
				if ("true".equals(params[1].toString().trim())) {
					btn_workstatus_read.setText("已读");
					btn_workstatus_read.setEnabled(false);
					Toast.makeText(WorkStatusDetailActivity.this, "已阅读", Toast.LENGTH_SHORT).show();
				} else {
					Toast.makeText(WorkStatusDetailActivity.this, "提交失败", Toast.LENGTH_SHORT).show();
				}
			}
			break;
		...
		case WorkStatusDetailActivity.WORKSTATUSDETAIL_FILE:
			if (!"".equals(params[1].toString().trim())&&params[1].toString().trim()!=null) {
                List<WorkStatusFileInfo> newInfos=parseJsonToWorkStatus(params[1].toString().trim());
                if (newInfos.size()>0) {
					workStatusFileInfos.clear();
					workStatusFileInfos.addAll(newInfos);
					workStatusFileAdapter=new WorkStatusFileAdapter(WorkStatusDetailActivity.this, workStatusFileInfos);
	...
	private void initView(){
		tv_workstatusname=(TextView) this.findViewById(R.id.tv_workstatusname);
		tv_workstatusmark=(TextView) this.findViewById(R.id.tv_workstatusmark);
		tv_workstatus_time=(TextView) this.findViewById(R.id.tv_workstatus_time);
		tv_workstatustime=(TextView) this.findViewById(R.id.tv_workstatustime);
		tv_workstatus_persons=(TextView) this.findViewById(R.id.tv_workstatus_persons);
		tv_workstatus_createtime=(TextView) this.findViewById(R.id.tv_workstatus_createtime);
		list_workstatus_appenddix=(ListView) this.findViewById(R.id.list_workstatus_appenddix);
		workStatusFileAdapter=new WorkStatusFileAdapter(WorkStatusDetailActivity.this, workStatusFileInfos);
		list_workstatus_appenddix.setAdapter(workStatusFileAdapter);
		list_workstatus_appenddix.setOnItemClickListener(new MyOnItemClickListener());
		btn_workstatus_read=(Button) this.findViewById(R.id.btn_workstatus_read);
		btn_workstatus_liuyan=(Button) this.findViewById(R.id.btn_liuyan);
		
		tv_workstatusname.setText(workStatusInfo.getTITLE());
		tv_workstatustime.setText(workStatusInfo.getNEWS_TIME().replace("T", " ").substring(0, 16));
		...
		tv_workstatus_createtime.setText(workStatusInfo.getCREATE_DATE().replace("T", " ").substring(0, 16));
		...
		btn_workstatus_liuyan.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				// TODO Auto-generated method stub
				Intent intent=new Intent(WorkStatusDetailActivity.this, WorkStatusLiuYanActivity.class);
	...
	private void getButtonStatus(){
		...
		PersonTask task=new PersonTask(PersonTask.WORKSTATUSINFODETAILACTIVITY_GET_WORKSTATUSINFO_STATUS, params);
	...
	private void getWorkStatusFile(){
		...
		data.put("Master_id", ""+workStatusInfo.getID());
		params.put("data", data);
		PersonTask task=new PersonTask(PersonTask.WORKSTATUSINFODETAILACTIVITY_GET_WORKSTATUSFILE, params);
		PersonService.newTask(task);
	}
	...
	private class MyOnItemClickListener implements OnItemClickListener{
		...
		public void onItemClick(AdapterView<?> arg0, View arg1, int position,
				long arg3) {
			// TODO Auto-generated method stub
			String filePath = ("/"+((WorkStatusFileInfo) workStatusFileAdapter.getItem(position))
					.getFILE_PATH().replaceAll(" ", "%20"));
	...
	private void btnCheckWorkStatusFair_OnClick(){
		...
		data.put("master_id", ""+workStatusInfo.getID());
		params.put("data", data);
		PersonTask task=new PersonTask(PersonTask.WORKSTATUSINFODETAILACTIVITY_SET_WORKSTATUSINFO_STATUS, params);
		PersonService.newTask(task);
	}
