...
public class WorkStatusMsgAdapter extends BaseAdapter{
	...
	public View getView(int position, View convertView, ViewGroup arg2) {
		...
		if (convertView == null) {
			convertView = LayoutInflater.from(mContext).inflate(R.layout.activity_msgboarddetile, null);
		...
		itemHodler.tv_msgboarddetile_name.setText(workStatusMsgInfo.getStaffName());
		itemHodler.tv_msgboarddetile_time.setText(workStatusMsgInfo.getCREATE_TIME().replace("T", " ").substring(0,19));
		itemHodler.tv_msgboarddetile_doc.setText(workStatusMsgInfo.getMSG());
		return convertView;
	}

	class QueryItemHodler {
		TextView tv_msgboarddetile_name,tv_msgboarddetile_time,tv_msgboarddetile_doc;
	}
