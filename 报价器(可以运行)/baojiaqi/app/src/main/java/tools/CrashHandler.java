	...
	private boolean handleException (Throwable ex) {
		...
		new Thread(){
			...
			public void run(){
				...
				Log.i("2017-5-17","很抱歉，程序出现异常，即将推出");
				Looper.loop();
			}
		}.start();
	...
	private String saveCrashInfo2File(Throwable ex){
		...
		try{
			...
			if(Environment.getExternalStorageState().equals(Environment.MEDIA_MOUNTED)){
				...
				MainActivity.verifyStoragePermissions(mContext);
				//mContext.verifyStoragePermissions();
				FileOutputStream fos = new FileOutputStream(path+fileName);//这里有错误
